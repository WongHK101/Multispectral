...
def reset_appearance_features(gaussians):
    with torch.no_grad():
    ...
    try:
        ...
        adam_cleared = False
    print(f"[INFO] ResetAppearanceFeatures: sh_zeroed=1 adam_state_cleared={1 if adam_cleared else 0}")


def project_features_to_tied_scalar_rgb(gaussians):
    if gaussians._features_dc is None or gaussians._features_rest is None:
        return
    with torch.no_grad():
        if gaussians._features_dc.ndim == 3 and gaussians._features_dc.shape[-1] == 3:
            dc_mean = gaussians._features_dc.mean(dim=2, keepdim=True)
            gaussians._features_dc.copy_(dc_mean.expand_as(gaussians._features_dc))
        if gaussians._features_rest.ndim == 3 and gaussians._features_rest.shape[-1] == 3:
            rest_mean = gaussians._features_rest.mean(dim=2, keepdim=True)
            gaussians._features_rest.copy_(rest_mean.expand_as(gaussians._features_rest))


def load_scene_manifest_payload(scene_root: str) -> dict:
    manifest_path = Path(scene_root) / "spectral_manifest.json"
...
def assert_rectified_band_scene(source_path: str, args) -> None:
    if getattr(args, "modality_kind", "rgb") != "band":
        return
    if not bool(getattr(args, "require_rectified_band_scene", False)):
        return
    payload = load_scene_manifest_payload(source_path)
    ...
    if rectification_status == "rectified" and scene_kind == "rectified_band":
        return
    raise RuntimeError(
        "Band stage-2 now requires a rectified band scene. "
        f"Got source_path={source_path!r}, rectification_status={rectification_status!r}, scene_kind={scene_kind!r}."
    )


def masked_l1_loss(network_output: torch.Tensor, gt: torch.Tensor, mask: torch.Tensor, eps: float = 1e-6) -> torch.Tensor:
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, ss_args=None):
    ...
    if getattr(args, "reset_appearance_features", False) and checkpoint and getattr(args, "start_checkpoint", None):
        reset_appearance_features(gaussians)
    ...
    if checkpoint and getattr(args, "start_checkpoint", None) and getattr(args, "tied_scalar_carrier", False):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        with torch.no_grad():
            ...
            if (iteration in saving_iterations):
                if getattr(args, "tied_scalar_carrier", False):
                    project_features_to_tied_scalar_rgb(gaussians)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ss_adaptive_nn", type=_str2bool, nargs="?", const=True, default=False)
    ...
    parser.add_argument("--ss_gate_densify", type=_str2bool, nargs="?", const=True, default=False)
    parser.add_argument("--ss_prune_before_thermal", type=_str2bool, nargs="?", const=True, default=False)
    parser.add_argument("--ss_prune_after_rgb", type=_str2bool, nargs="?", const=True, default=False)
    parser.add_argument("--debug_gaussian_stats", type=_str2bool, nargs="?", const=True, default=False)
    parser.add_argument("--clamp_scale_max", type=float, default=None)
    parser.add_argument("--clamp_scale_after_densify", type=_str2bool, nargs="?", const=True, default=False)
    parser.add_argument("--clamp_scale_after_rgb_final", type=_str2bool, nargs="?", const=True, default=False)
    parser.add_argument("--thermal_reset_features", type=_str2bool, nargs="?", const=True, default=False)
    parser.add_argument("--sgf_disable", type=_str2bool, nargs="?", const=True, default=False)
    parser.add_argument("--baseline_modules_off", type=_str2bool, nargs="?", const=True, default=False)
    parser.add_argument("--baseline_restore_ssp", type=_str2bool, nargs="?", const=True, default=False)
    parser.add_argument("--baseline_restore_stt", type=_str2bool, nargs="?", const=True, default=False)
    ...
    if args.reset_appearance_features is None and args.thermal_reset_features:
        args.reset_appearance_features = True

    if args.modality_kind == "band":
        args.single_band_mode = True

    if args.stage2_mode == "band_transfer":
        if args.modality_kind == "rgb":
            args.modality_kind = "band"
        args.single_band_mode = True
        if args.single_band_replicate_to_rgb is None:
            args.single_band_replicate_to_rgb = True
        if args.reset_appearance_features is None:
            args.reset_appearance_features = True
        if args.freeze_geometry is None:
            args.freeze_geometry = True
        if args.freeze_opacity is None:
            args.freeze_opacity = True
        if args.tied_scalar_carrier is None:
            args.tied_scalar_carrier = True
        if args.require_rectified_band_scene is None:
            args.require_rectified_band_scene = True
        if args.use_validity_mask is None:
            args.use_validity_mask = True
        if args.start_checkpoint:
            ...
            args.densify_from_iter = 999999
            args.densify_until_iter = 0
            args.densification_interval = 999999
            args.opacity_reset_interval = 999999

    if args.single_band_replicate_to_rgb is None:
        args.single_band_replicate_to_rgb = True if args.single_band_mode else False
    if args.reset_appearance_features is None:
        args.reset_appearance_features = False
    if args.freeze_geometry is None:
        args.freeze_geometry = False
    if args.freeze_opacity is None:
        args.freeze_opacity = False
    if args.tied_scalar_carrier is None:
        args.tied_scalar_carrier = False
    if args.require_rectified_band_scene is None:
        args.require_rectified_band_scene = False
    if args.use_validity_mask is None:
        args.use_validity_mask = False
    if args.freeze_geometry:
        args.position_lr_init = 0.0
        args.position_lr_final = 0.0
        args.scaling_lr = 0.0
    ...
    if args.start_checkpoint and (not opacity_flag_set):
        if args.freeze_opacity:
            args.opacity_lr = 0.0
        elif args.baseline_restore_stt:
    ...
    assert_rectified_band_scene(args.source_path, args)
