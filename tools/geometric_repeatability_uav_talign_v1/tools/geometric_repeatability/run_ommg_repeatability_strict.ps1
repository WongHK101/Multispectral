param(
    ...
    [string]$OMMGPython = "D:\anaconda\envs\TGS-OMMG\python.exe",
    [string]$OMMGRepo = "E:\3DGS\TGS\Thermal-Gaussian-OMMG",
    [string]$StrictRunRoot = "F:\databackup\xr6\output\GeometricRepeatability\Building\M01_Strict_v1",
    [string]$OutRoot = "F:\databackup\xr6\output\GeometricRepeatability\CrossMethod\ThermalGaussian_OMMG\Building\Strict_v1",
...
function Prepend-PythonPathEntries {
    param(
        [Parameter(Mandatory = $true)][string[]]$Entries
    )
    $valid = $Entries | Where-Object { $_ -and (Test-Path -LiteralPath $_) }
    if ($valid.Count -eq 0) {
        return
    }
    $existing = $env:PYTHONPATH
    if ([string]::IsNullOrWhiteSpace($existing)) {
        $env:PYTHONPATH = ($valid -join ";")
    }
    else {
        $env:PYTHONPATH = (($valid -join ";") + ";" + $existing)
    }
}
...
function Use-OMMGPythonEnv {
    Restore-PythonEnv
    Prepend-CondaEnvPath -PythonPath $script:OMMGPython
    Prepend-PythonPathEntries -Entries $script:OMMGPythonPathEntries
}
...
$script:OriginalPythonPath = $OriginalPythonPath
$script:OMMGPython = $OMMGPython
$script:OMMGPythonPathEntries = @(
    $OMMGRepo,
    (Join-Path $OMMGRepo "submodules\\diff-gaussian-rasterization\\build\\lib.win-amd64-cpython-37"),
    (Join-Path $OMMGRepo "submodules\\simple-knn\\build\\lib.win-amd64-cpython-37")
)
$status = [ordered]@{
    scene = $SceneName
...
try {
    ...
    Assert-Exists -PathValue $OMMGPython -Label "OMMG python"
    Assert-Exists -PathValue $OMMGRepo -Label "OMMG repo"
    ...
    Invoke-Step -Name "Train Odd OMMG" -Body {
        Write-Status -Stage "train_odd"
        if (Test-Path -LiteralPath (Join-Path $OddModel "chkpnt30000.pth")) {
            Write-Host "[SKIP] Odd OMMG checkpoint already exists."
            return
        }
        Use-OMMGPythonEnv
        Invoke-NativePython -ExePath $OMMGPython `
            -WorkingDirectory $OMMGRepo `
            -Label "Odd OMMG training" `
            -Arguments @(
                (Join-Path $OMMGRepo "train-OMMG.py"),
                "-s", $OddDataset,
                "-m", $OddModel,
                "--iterations", "30000",
    ...
    Invoke-Step -Name "Train Even OMMG" -Body {
        Write-Status -Stage "train_even"
        if (Test-Path -LiteralPath (Join-Path $EvenModel "chkpnt30000.pth")) {
            Write-Host "[SKIP] Even OMMG checkpoint already exists."
            return
        }
        Use-OMMGPythonEnv
        Invoke-NativePython -ExePath $OMMGPython `
            -WorkingDirectory $OMMGRepo `
            -Label "Even OMMG training" `
            -Arguments @(
                (Join-Path $OMMGRepo "train-OMMG.py"),
                "-s", $EvenDataset,
                "-m", $EvenModel,
