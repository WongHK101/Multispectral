...
class CameraInfo(NamedTuple):
    ...
    height: int
    is_test: bool
    modality_type: str = "rgb"
    band_name: str = ""
    carrier_mode: str = "native_rgb"
    paired_group_id: str = ""
...
def _load_scene_manifest_payload(scene_root: str) -> Dict[str, object]:
...
def _load_scene_manifest(scene_root: str) -> Dict[str, Dict[str, object]]:
...
def readColmapCameras(cam_extrinsics, cam_intrinsics, depths_params, images_folder, depths_folder, test_cam_names_list,
                      manifest_by_name=None, scene_manifest=None):
    cam_infos = []
    manifest_by_name = manifest_by_name or {}
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        meta_entry = manifest_by_name.get(image_name, {})
        modality_type = str(meta_entry.get("modality_type", "rgb") or "rgb")
        band_name = str(meta_entry.get("band_name", "") or "")
        carrier_mode = str(meta_entry.get("carrier_mode", "native_rgb") or "native_rgb")
        paired_group_id = str(meta_entry.get("paired_group_id", "") or "")
        image_meta = meta_entry.get("metadata", {}) if isinstance(meta_entry, dict) else {}
        validity_mask_path = str(meta_entry.get("validity_mask_path", "") or "")
        scene_kind = str(meta_entry.get("scene_kind", "") or scene_manifest.get("scene_kind", "") or "")
        rectification_status = str(meta_entry.get("rectification_status", "") or scene_manifest.get("rectification_status", "") or "")

        cam_info = CameraInfo(uid=uid, R=R, T=T, FovY=FovY, FovX=FovX, depth_params=depth_params,
                              image_path=image_path, image_name=image_name, depth_path=depth_path,
                              width=width, height=height, is_test=image_name in test_cam_names_list,
                              modality_type=modality_type, band_name=band_name, carrier_mode=carrier_mode,
                              paired_group_id=paired_group_id, image_meta=image_meta,
                              validity_mask_path=validity_mask_path, scene_kind=scene_kind,
...
def readColmapSceneInfo(path, images, depths, eval, train_test_exp, llffhold=8):
    ...
    manifest_by_name = _load_scene_manifest(path)
